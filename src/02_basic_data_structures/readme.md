...
Используются только пользовательские типы на основе встроенных:

```ocaml
{{#include ./code/hw_w2s4_db.ml:db_demo_code}}
```

Особенность этой реализации — отсутствие деструктивных изменений (non destructive program). Получился purely functional DB engine. Каждый раз возвращается новая БД, предыдущая не меняется.

> Side-effects considered harmful.

Преимущества такого подхода:

- функции возвращают только новые значения и не меняют уже существующие;
- выполнение функции не зависит от внешней среды, только от её аргументов (как в математике);
- поэтому математические подходы можно использовать;
- программа в функциональном стиле не зависит от состояния, поэтому может быть использована когда угодно (не нужно дать, пока что-то в стейте просрётся).

Всё это делает программы в ФП-стиле более composable: проще и удобнее собирать цельную программу из независимых кусочков.

Эта конкретная реализация БД далека от идеала, потому что:

- запросы возвращают избыточную информацию, потому что используются только пользовательские типы на основе примитивов (при поиске контакта приходит вся база, а при добавлении контакта приходит контакт, потому что все результаты должны быть типа `(status : Bool, db : database, c : contact)` для соблюдения инварианта). Тип, возвращаемый `engine` должен быть `union`: либо контакт, либо БД.
- БД каждый раз целиком дублируется. Можно использовать более пригодные для таких операций структуры данных, чтобы общие части не дублировались.

Эти косяки можно победить с помощью [алгебраических типов](../03_more_advanced_data_structures/index.html).
